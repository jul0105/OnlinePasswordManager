...
/// Date : 23.05.2021
///
/// Password validation, verification and store
...
pub fn validate(password: &str) -> Result<(), String> {
    if !check_password_policy(password) {
        return Err("Invalid password. Password must be between 8 and 64 characters long.".to_string());
    }
...
pub fn verify(current_hash: &str, new_password: &[u8]) -> bool {
    ARGON2.verify_password(new_password, &PasswordHash::new(&current_hash).unwrap()).is_ok()
}
...
mod tests {
    ...
    fn same_password_should_have_different_hash_with_salt() {
        let hash1 = store(b"1234");
        let hash2 = store(b"1234");
    ...
    fn password_hash_and_verify() {
        let hash_and_verify = |pass: &[u8]| assert!(verify(store(pass).as_str(), pass));

        hash_and_verify(b"");
        hash_and_verify(b"test");
        hash_and_verify(b"123456789");
        hash_and_verify(b"RDK*HdaLT*N94ckaReuHYcqWSW*FozVojA^$DRJeRMWz!PNa&sJqtz#tJp@q85swwRD^JkMx3ft4n#MeQ5ACPB*LA68ryuVyzVan");
        hash_and_verify(b"\x00");
    }

    #[test]
    fn verify_different_password() {
        let hash_and_verify = |pass1: &[u8], pass2: &[u8]| assert!(!verify(store(pass1).as_str(), pass2));

        hash_and_verify(b"", b"1234");
        hash_and_verify(b"test", b"tes");
        hash_and_verify(b"123456789", b"abcdefg");
        hash_and_verify(b"RDK*HdaLT*N94ckaReuHYcqWSW*FozVojA^$DRJeRMWz!PNa&sJqtz#tJp@q85swwRD^JkMx3ft4n#MeQ5ACPB*LA68ryuVyzVan", b"");
        hash_and_verify(b"\x00", b"\x01");
    }

    #[test]
    fn verify_corrupted_hash() {
        let password = b"123456789";
        let hash = store(password);
    ...
    fn test_validate() {
        ...
        assert_eq!(validate("RDK*HdaLT*N94ckaReuHYcqWSW*FozVojA^$DRJeRMWz!PNa&sJqtz#tJp@q85s").is_ok(), true); // 64
        assert_eq!(validate("RDK*HdaLT*N94ckaReuHYcqWSW*FozVojA^$DRJeRMWz!PNa&sJqtz#tJp@q85sw").is_ok(), true); // 64
        assert_eq!(validate("RDK*HdaLT*N94ckaReuHYcqWSW*FozVojA^$DRJeRMWz!PNa&sJqtz#tJp@q85sww").is_ok(), false); // 65
    ...
    fn test_check_password_policy() {
        ...
        assert_eq!(check_password_policy("012345678901234567890123456789"), true); // 30
        assert_eq!(check_password_policy("012345678901234567890123456789012345678901234567890123456789012"), true); // 63
        assert_eq!(check_password_policy("0123456789012345678901234567890123456789012345678901234567890123"), true); // 64
        assert_eq!(check_password_policy("01234567890123456789012345678901234567890123456789012345678901234"), false); // 65
        assert_eq!(check_password_policy("0123456789012345678901234567890123456789012345678901234567890123456789"), false); // 70
    }
    ...
    fn test_check_password_strength() {
        ...
        assert_eq!(check_password_strength("MorningVerifyTelevisionWood").is_ok(), true);
